...
// integer arithmetic)..
static inline void idct_1D_u32(int32_t *in, int instride, int32_t *out, int outstride)//自己用
{
    for (int x = 0; x < 8; x++)
        out[x*outstride] = 0;
...
void pjpeg_idct_2D_u32(int32_t in[64], uint8_t *out, uint32_t outstride)//本文档中仅有一处
{
    int32_t tmp[64];
...
void pjpeg_idct_2D_nanojpeg(int32_t in[64], uint8_t *out, uint32_t outstride) //本文档中仅有一处
{
    int coef;
